    ...
    for rec in master:
        ...
        if desp_dt:
            ...
            if coach_fy == fy:
                ...
                if family == "ICF":
                    count += 1
                    print(f"{count}. Coach: {rec.get('coachno')}, Desc: {coach_desc}, Desp: {desp_str}, Repair: {decode_repair(detail.get('repairid'))} ({detail.get('repairid')})", flush=True)
