...
for idx, line in enumerate(lines):
    if "async function loadLivePosition()" in line or "function loadLivePosition()" in line:
        start = idx
        break

if start != -1:
    print("Found loadLivePosition at line:", start + 1)
    for i in range(start, min(len(lines), start + 100)):
        print(f"{i+1}: {lines[i]}", end="")
else:
    print("loadLivePosition definition not found")
