...
for idx, line in enumerate(lines):
    if "async function loadDashboard()" in line or "function loadDashboard()" in line:
        start = idx
        break

if start != -1:
    print("Found loadDashboard at line:", start + 1)
    for i in range(start, min(len(lines), start + 80)):
        print(f"{i+1}: {lines[i]}", end="")
else:
    print("loadDashboard definition not found")
